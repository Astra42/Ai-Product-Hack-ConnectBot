...
back_to_profile_target = InlineKeyboardMarkup(inline_keyboard=[[
    InlineKeyboardButton(text="📝 Назад к анкете", callback_data="set_profile"),
    InlineKeyboardButton(text="🔎 Искать!", callback_data="search_interlocutor")
]])

def get_watch_next_kb_buttons():
    watch_next_kb = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="✏ Изменить кого ищу")],
        [KeyboardButton(text="🍺 / 🍷")],
...
def get_watch_next_kb(num: int):
    watch_next_kb = InlineKeyboardMarkup(
        inline_keyboard=[[
            InlineKeyboardButton(text="✏ Изменить кого ищу", callback_data="edit_target"),
